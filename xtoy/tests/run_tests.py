...
def apply_toy_on(X, y, cl_or_reg=None, n=500):
    n = min(len(y), n)
...
def test_iris_data():
    ...
    assert apply_toy_on(X, y) > 0.8


def test_boston_data():
    boston = load_boston()
    X, y = boston.data, boston.target
    assert apply_toy_on(X, y) > -100
...
def test_newsgroup_data():
    ...
    assert apply_toy_on(X, np.array(y), cl_or_reg='classification') > 0.0
